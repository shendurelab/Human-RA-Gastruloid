#This script import and process scRNA-seq from 24,48,72 and 96 hours of orginal human gastruloids (Moris et al., 2021)
#The processed .RDS files from this script are provided at National Center for Biotechnology Information (NCBI) Gene Expression Omnibus (GEO) under accession numbers GSE208369
...
RNA.qc <- function(timepoint,dir_path=""){
    ...
    rna_dir_path=paste0("/net/shendure/vol10/projects/hGastruloid/count_10X_RNA_",
                        timepoint,
                        "_gastruloid/outs/filtered_feature_bc_matrix")  #The barcode matrix folders are outputs from 10x pipeline. Fastq is provided in GEO.
        }else{
    ...
    #Attach doublet info 
    source_python("./scrub_doublets.py")
...
hGastruloids_24h_rna <- RNA.qc("24h")
...
#Remove cluster of low complexity (GeneperUMI) and cluster with Ribosomal and mitochondrial marker genes
...
hGastruloids_48h_rna <- RNA.qc("48h")
...
#Remove cell cycle, mitochondrial, non-coding RNA and ribosomal RNA genes that bias clustering results
#Remove cluster of low complexity
counts <- GetAssayData(hGastruloids_96h_rna[,hGastruloids_96h_rna$seurat_clusters!=13], assay = "RNA")
counts <- counts[-(which(str_detect(rownames(counts), "^HIST|^MT-|^TOP|^CDK|^CCN|^CDC|^CCDC|^MKI|MALAT|^NUSAP|^SMC|^CENP|^UBE|^SGO|^ASPM|^PLK|^KPN|^RP|^PTTG|^SNHG|^CK|^AUR^|^BUB|^KIF|^KCNQ|^SMO|^HMG|^S100|^LINC|^ATP|^IGFBP|^HSP|^FOS|^JUN"))), ]
