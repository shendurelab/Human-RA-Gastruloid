#This script performs NNLS-based lineage specific molecular staging
...
#Human gastruloids(provided in GEO under accession numbers GSE208369)
hGastruloids_RA_120h_rna_merged <- readRDS(paste0(geo_dir,'RA_hGas_120h.RDS'))
Idents(hGastruloids_RA_120h_rna_merged) = hGastruloids_RA_120h_rna_merged$cell_type
hGastruloids_RA_96h_rna = readRDS(paste0(geo_dir,'RA_hGas_96h.RDS'))
hGastruloids_96h_rna = readRDS(paste0(geo_dir,'hGas_96h.RDS'))
hGastruloids_RA_LDN = readRDS(paste0(geo_dir,'RA_LDN.RDS'))
...
res_RA_LDN <- pseudobulk_hG(hGastruloids_RA_LDN,'RA_LDN')
...
homologs=Reduce(intersect, list(rownames(res_mG),rownames(res_moca_e85),rownames(res_moca_e95),rownames(res_moca_e105))) 
...
df_tmp = df_tmp%>%arrange(day)
dat = dat[df_tmp$Anno,c('Endothelium','Endoderm','NMP','Paraxial MD','Differentiation front','Somite','PSM','PGC-like','Cardiac','Neural tube','Differentiated somite')]
...
# #RA gastruloids 96hr vs MOCA
homologs=Reduce(intersect, list(rownames(res_RA_hG_96h),rownames(res_moca_e85),rownames(res_moca_e95),rownames(res_moca_e105))) 
res_RA_hG_96h_filtered=res_RA_hG_96h[homologs,]
res_moca_e85_filtered=res_moca_e85_filtered[homologs,]
res_moca_e95_filtered=res_moca_e95_filtered[homologs,]
res_moca_e105_filtered=res_moca_e105_filtered[homologs,]
res_moca <-cbind(res_moca_e85_filtered,cbind(res_moca_e95_filtered,res_moca_e105_filtered))
nnls_result <- correlation_analysis_bidirection(res_RA_hG_96h_filtered,res_moca,fold.change = 2.5, top_gene_num = 200, spec_gene_num = 200)
nnls_output_all <- correlation_analysis_cell_type_selection(nnls_result)
dat <- nnls_output_all[[1]]
df_tmp = data.frame(Anno=rownames(dat), day=str_split_fixed(rownames(dat),':',2)[,1])
df_tmp$day = ordered(df_tmp$day,levels=c('E8.5b','E9.5','E10.5'))
df_tmp = df_tmp%>%arrange(day)
dat = dat[df_tmp$Anno,c('Differentiation front','Somite','Neural tube','Neural progenitor','Neural crest',
                       'DE','IM meso./Renal progenitor','Renal epithelium','Cardiac meso.')]
options(repr.plot.width=30, repr.plot.height=10)
pheatmap(t(dat), color = colorRampPalette(c("navy", "white", "firebrick3"))(50),fontsize = 18,
        cluster_rows=F, cluster_cols=F,scale='row')

#Original gastruloids 96hr vs MOCA
homologs=Reduce(intersect, list(rownames(res_hG_96h),rownames(res_moca_e85),rownames(res_moca_e95),rownames(res_moca_e105))) 
res_hG_96h_filtered=res_hG_96h[homologs,]
res_moca_e85_filtered=res_moca_e85_filtered[homologs,]
res_moca_e95_filtered=res_moca_e95_filtered[homologs,]
res_moca_e105_filtered=res_moca_e105_filtered[homologs,]
res_moca <-cbind(res_moca_e85_filtered,cbind(res_moca_e95_filtered,res_moca_e105_filtered))
nnls_result <- correlation_analysis_bidirection(res_hG_96h_filtered,res_moca,fold.change = 2.5, top_gene_num = 200, spec_gene_num = 200)
nnls_output_all <- correlation_analysis_cell_type_selection(nnls_result)
dat <- nnls_output_all[[1]]
df_tmp = data.frame(Anno=rownames(dat), day=str_split_fixed(rownames(dat),':',2)[,1])
df_tmp$day = ordered(df_tmp$day,levels=c('E8.5b','E9.5','E10.5'))
df_tmp = df_tmp%>%arrange(day)
dat = dat[df_tmp$Anno,c('Gut','Advanced Meso.','NMP','Node','Somite','PSM','Differentiation front','Cardiac')]
options(repr.plot.width=30, repr.plot.height=10)
pheatmap(t(dat), color = colorRampPalette(c("navy", "white", "firebrick3"))(50),fontsize = 18,
        cluster_rows=F, cluster_cols=F,scale='row')

# LDN gastruloids 120hr vs MOCA
homologs=Reduce(intersect, list(rownames(res_RA_LDN),rownames(res_moca_e85),rownames(res_moca_e95),rownames(res_moca_e105))) 
res_RA_LDN_filtered=res_RA_LDN[homologs,]
res_moca_e85_filtered=res_moca_e85_filtered[homologs,]
res_moca_e95_filtered=res_moca_e95_filtered[homologs,]
res_moca_e105_filtered=res_moca_e105_filtered[homologs,]
res_moca <-cbind(res_moca_e85_filtered,cbind(res_moca_e95_filtered,res_moca_e105_filtered))
nnls_result <- correlation_analysis_bidirection(res_RA_LDN_filtered,res_moca,fold.change = 2.5, top_gene_num = 200, spec_gene_num = 200)
nnls_output_all <- correlation_analysis_cell_type_selection(nnls_result)
dat <- nnls_output_all[[1]]
df_tmp = data.frame(Anno=rownames(dat), day=str_split_fixed(rownames(dat),':',2)[,1])
df_tmp$day = ordered(df_tmp$day,levels=c('E8.5b','E9.5','E10.5'))
